...
const AllShopsPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true)
        const routesRes = await axios.get('/api/routes/my-route')
  ...
  return (
    <Box sx={{ pb: 12, bgcolor: 'customColors.bodyBg', minHeight: '100vh' }}>
      ...
      <Box sx={{ p: 6, mt: -10 }}>
        <TextField
          fullWidth
          placeholder='Which shop are you visiting?'
          value={searchQuery}
        ...
        <Grid container spacing={4}>
          {filteredShops.map((shop) => (
            <Grid item xs={12} key={shop._id}>
              <ShopCardItem onClick={() => router.push(`/orders/create?shopId=${shop._id}&routeId=${shop.routeId}`)}>
        ...
        {filteredShops.length === 0 && (
          <Box sx={{ py: 20, textAlign: 'center', opacity: 0.5 }}>
            ...
            <Typography sx={{ mt: 2 }}>No shops found matching your search</Typography>
          </Box>
        )}
      </Box>
    </Box>
